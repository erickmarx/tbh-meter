...
def read_live_exp(reader, uf):
    ...
    1.00.20 killed the PLAIN fakeValue decoy; the real value is behind the
    Obscured cipher. This decodes it using the byte-swap algorithm discovered
    via Capstone disassembly of GameAssembly.dll (2026-06-24).
    ...
    SPECIFIC to 1.00.20 - cipher rotates per build; breaks on game update."""
    try:
        if not uf:
            return None
    ...
    try:
        ...
        for i in range(n):
            ...
            if hero_cat is not None and hk not in hero_cat:
                continue
            # LEVEL from the SAVE (live decoy dead). EXP from LIVE ObscuredFloat decode
            # (1.00.20 byte-swap algorithm); None on failure -> honest save fallback.
